...
public class Ex25_array {
	...
	private static void m11() {
		...
		System.out.println(Arrays.toString(ns)); //덤프 메소드 > 배열 상태 확인용으로 사용!!! XXX
		
	}
	
	//배열의 상태를 문자열로 바꾸어 반환 > 덤프(dump)
	...
	private static void m9() {
		...
		//배열 자동 초기화
		//-배열(참조형)은 값형과 다르게 생성 직후에 사용자(개발자)가
		// 아무것도 안해도 자동으로배열에 초기화가 된다.
	...
	private static void m6() throws Exception{
		...
		//성적 처리 프로그램 
		//- 배열의 길이? > 교장 > 300명 > 320~330명
		
		
		//자바의 배열은 길이는 불변이지만, 동적할당은 가능하다.
		
		//정적할당 > 컴파일 때 이미 배열의 길이가 정해진다.
		int[] nums = new int[3];
		
		//동적할당 > 컴파일 때는 배열의 길이를 모름 > 런타임 때 결정된다.
	...
	private static void m4() {
		...
		for (int i=0; i<3; i++) { //3을 4로 잘못 쓰면 위와 같은 에러가 뜬다.
