...
public class Ex44_ToString {
	...
	public static void main(String[] args) {
		...
		System.out.println(t2.info()); //객체의 상태를 한번에 확인!!
		...
		//모든 주소값을 저장하는 변수는 int형(4byte)
		//String변수도 주소값을 저장하는 상자 자체는 int형이다. > 메모리 주소 > int 사용
		//com.test.java.obj.inheritance.Time > 객체의 자료형(패키지.클래스)
		//@
		//2aafb23c > 숫자 > 메모리 주소값 > 해시코드(HashCode)
		...
		//t1과 now의 toString 차이가 나는 이유
		//now.toString() : String - Date(오버라이드)
		//Date클래스에서 toString을 재정의 했다는 뜻
		...
		//- 모든 객체는 toString() 소유
		//- (toString)어떤 코드를 재정의? > 자신이 소유한 데이터(멤버 변수)의 값을 문자열로 돌려주는 코드
		//							> 덤프(dump)
		//- toString은 규칙 역할 > 이런걸 만들어라
...
class Time {
	...
	//덤프 > 객체의 상태를 확인하기 쉽도록 만든 메소드 > 개발자(나)를 위해서
	public String info() {
...
//	@Override
//	public String toString() { //toString을 재정의 > 이상하게 나오던 값이 제대로 나옴
//							   //- line 36, 37
//		
//		//최소 권장사항 > 객체의 데이터를 알아볼 수 있게 문자열..
//		
//		return this.hour + ":" + this.min;
//	}
	...
	//마우스 우측 > source > Generate toString()
	//개발자만 보는 정보
	@Override
