...
public class Dummy {
	...
	private static void createStudent() throws Exception { //원래 try catch로 해야함
		...
		//System.out.println(rnd.nextInt(10) + 1); //1~9
		
		
		//번호, 이름, 나이, 성별, 주소
 	    //ex) 1, 홍길동, 15, 1, 서울특별시 강남구 역삼동
		...
		String[] address3 = {"압구정동", "천호동", "등촌동", "대치동", "역삼동"};
		...
		for(int i=0; i<size; i++) {
			...
			int age = rnd.nextInt(3) + 14; //14~16
			int gender = rnd.nextInt(2) + 1; //1~2
	...
	private static void createScore() throws Exception{
 	    ...
 	    //학생번호, 국어, 영어, 수학 (학생번호는 중복이 없어서 기준으로 잡음)
 	    //1,100,80,90
		
		//학생 18명(2명 삭제) > 성적 3회 분량
		...
		for (int j=0; j<3; j++) {
			...
			while((line = reader.readLine()) != null) {
				...
				//System.out.println(seq); //번호 지우고 존재하는 번호만 읽어보기
				
				int kor = rnd.nextInt(51) + 50; //50~100
				int eng = rnd.nextInt(51) + 50; //50~100
				int math = rnd.nextInt(51) + 50; //50~100
