...
public class Work {
	...
	private Scanner scan; //반복해서 입력하기 때문에 전역변수로 선언해준다.
	
	public Work() {
		this.scan = new Scanner(System.in); //생성자 안에 초기화 해준다.
	}
	...
	private void delete() {
		...
		for (Student s : Data.slist) {
			...
			if (s.getSeq().equals(input)) {
				result = s; 
				//Data.slist.remove(s);//Arraylist의 remove에 인자로 객체자체를 넣는다.
				break;
			}			
		}
	...
	private void edit() {
		...
		if (result != null) {
			...
			System.out.print("수정: "); //건너뛰기 엔터 라는 멘트 있다 치고
			String name = scan.nextLine();
			...
			System.out.print("수정(1.남자, 2여자): "); //건너뛰기 엔터 라는 멘트 있다 치고
			String gender = scan.nextLine();
	...
	private void view() {
		...
		String input = scan.nextLine(); //학생 번호(seq)
	...
	public void search() {
		...
		for (Student s : Data.slist) {
			...
			if(s.getName().contains(input) 
				|| s.getAddress().replace(" ", "").contains(input.replace(" ", ""))) { //주소나 이름에 input이 들어가면 출력	
				System.out.printf("%4s\t%s\t%4s\t%3s\n"
								, s.getSeq()
								, s.getName()
								, s.getAge()
