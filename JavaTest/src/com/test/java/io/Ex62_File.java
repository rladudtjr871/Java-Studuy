...
public class Ex62_File {
	...
	private static void m15() {
		...
		//파일 11,870 폴더 2,498 크기 805,197,475 바이트
	...
	private static void countFile(File dir) {
		...
		for (File subdir : list) {
			if (subdir.isDirectory()) {
				//count++; //2번을 주석 처리하고 이 부분을 추가하면 폴더 개수
...
//			for(String f : list) {
//				
//				//이름 사용 > File객체 생상 > isFile(), isDirectory()
//				System.out.println(f);
//				
//				//file과 file이 아닌 것을 구분
//				File file = new File(dir.getAbsoluteFile() + "\\" + f);
//				System.out.println(file.isFile()); //파일은 true
//				System.out.println();
//				
//				
//			}
...
//		}
	...
	private static void m2() {
		...
		if (dir.exists()) {
			...
			System.out.println(dir.lastModified()); //1649031809554
	...
	private static void m1() {
		...
		//System.out.println(file.exists()); //파일이 존재하니? > true
		
		//파일 조작시 파일이 존재(true)일때만 실행되도록 한다.
		if(file.exists()) {
			...
			System.out.println(file.length());	//26 > 파일 크기(byte)
			
			System.out.println(file.getAbsolutePath()); //C:\class\java\file\data.txt > 절대 경로
			System.out.println(file.getPath()); //C:\class\java\file\data.txt > 경로
			
			System.out.println(file.lastModified()); //1649031848213 > tick값
			System.out.println(file.isHidden()); //숨김파일이냐? > false
			
			//tick -> 년월일시분초
			Calendar c1 = Calendar.getInstance();
			System.out.println(c1.getTimeInMillis()); // tick값을 가져오는 메소드
			c1.setTimeInMillis(file.lastModified()); //c1을 file의 수정시간 tick값으로 수정
