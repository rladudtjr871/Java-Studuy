...
public class Ex27_array {
	...
	private static void m6() {
		...
		//변수값의 초기값 > 실패하거나 아무일도 안 일어났을 때 가져야될 값으로 초기화한다.
		//              ,피해가 가장 적을 것 같은 값
		...
		if (result) {
			...
			System.out.println(search + " 찾지 못했습니다..");
		}
	...
	public static boolean contains(String[] member, String search) { //있는지 찾기
	...
	public static int indexOf(String[] member, String search) { //어디 있는지 찾기
	...
	private static void m5() {
		...
		System.out.println(Arrays.toString(nums)); //왼쪽 시프트
		
		nums[nums.length-1] = 0;
		
		System.out.println(Arrays.toString(nums)); //마지막 값 수정
	...
	private static void m3() {
		...
		//중복 없는 임의의 로또 번호 6개를 만드시오 문제 (1~45)
		...
		for (int i=0; i<lotto.length; i++) {
			...
			//생선한 난수를 배열에 넣기 전에 > 기존의 숫자와 중복되는지 검사!
			
			for (int j=0; j<i; j++) { //기존의 숫자들
				
				if (lotto[j] == n) { //중복된 숫자!
					isDuplicate = true;
					i--; //중복일 경우 해당 i회차는 무효처리 > 빈 칸이 생기지 않도록
		...
		System.out.println(end - begin + "ns"); //소요시간
	...
	private static void m1() {
		...
		//2차원 배열
		//= new int[2][3]
		System.out.println(nums2.length); //행의 개수 (2)
