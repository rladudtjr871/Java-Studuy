...
public class Ex09_Casting {
	...
	public static void main(String[] args) {
		...
		/*
		  ...
		  2. 명시적 형변환(= 강제 타입 변환)
		  	  ...
		  	  - 경우에 따라 다르다. > 작은형이 소화할 수 있는 범위의 원본값이면 복사가 가능하고.
		  	    작은형이 소화할 수 없는 범위의 원본값이면 손실분이 생긴다. > 성공/실패
		  	  - 단 개발자의 주의를 요구함
		...
		s1 = b1;
		//(자료형)피연산자: 형변환 연산자 > 피연산자의 자료형을 ()안에 자료형으로 바꿔서 반환해라
		//s1 = (short)b1;
		...
		//1칸 = 2칸
		//형변환 연산자를 생략할 수 없다.
		b2 = (byte)s2; //개발자에게 경고하는 것! 틀릴 수 있으니까!
		...
		//*** 오버플로우가 발생할만한 상황은 미리 예측하여 관리해야한다.
		...
		//float = double
		//작은형(4) = 큰형(8)
		float f1 = 3.14F; //서비스 문법x > (float)을 적어야하지만 접미어에 F를 넣는걸로 대신함
		...
		//- boolean > 형변환 대상 불가능
		//- 값형과 참조형 간에는 형변환이 젏대 불가능
		
		//"100"(String) -> 100(int)  X
		//100(int) -> "100"(String) X
		...
		//결론 : char를 숫자로 형변환하려면 반드시 int 이상의 자료형으로 변환해야 한다.
		//(65335까지 표현할 수 있는 자료형이 필요)
