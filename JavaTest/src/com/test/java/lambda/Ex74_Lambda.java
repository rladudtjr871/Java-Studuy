...
public class Ex74_Lambda {
	...
	private static void m6() {
		...
		//c1 + c2 + c4
		Consumer<User> c4 = user -> System.out.println("직급: " + user.getPositon());
		...
		Function<Integer,Integer> f8 = f6.andThen(f7); //f6 > f7 //f6 먼저 실행
		Function<Integer,Integer> f9 = f7.compose(f6); //f6 + f7 //매개변수 먼저 실행
	...
	private static void m3() {
		...
		Function<User,String> f3 = user -> {
			if (user.getPositon().equals("사원") || user.getPositon().equals("대리")) {
				return "사원급";
			} else {
				return "간부급";
			}
	...
	private static void m1() {
		...
		Consumer<User> c4 = user -> {
			...
			System.out.println("직급: " + user.getPositon());
		};
...
//함수형 인터페이스(목적: 람다식을 저장하려고 만든 인터페이스, 문법 차이x)
// > 함수형 인터페이스는 반드시 추상 메소드를 딱 1개만 가질 수 있다.
//> 이 1개의 추상 메소드가 곧 람다식(익명 메소드)이 되기 때문에..
@FunctionalInterface   //1. 남에게 역할 알리기 위해 2. 문법체크(추상 메소드가 1개인지)
interface MyConsumer { //*MyConsumer is not a functional interface
	void test(int num);
	//void call(int test); //*은 얘로 인해 생기는 오류
}
